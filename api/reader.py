...
async def create_reader(reader: ReaderCreate, db: AsyncSession = Depends(get_session), current_user = Depends(get_current_user)):
    """Docstring"""
    reader_unit = await db.execute(select(Reader).where(Reader.email == reader.email))
    db_reader = reader_unit.scalars().first()
    if db_reader:
        raise HTTPException(status_code=HTTPStatus.CONFLICT, detail='Пользователь с такими данными уже есть в системе.')
...
async def list_readers(db: AsyncSession = Depends(get_session), current_user = Depends(get_current_user)):
    """Docstring"""
    result = await db.execute(select(Reader))
    return result.scalars().all()

@reader.get('/{reader_id}', response_model=ReaderResponse)
async def get_reader(reader_id: int, db: AsyncSession = Depends(get_session), current_user = Depends(get_current_user)):
    """Docstring"""
    reader_unit = await db.execute(select(Reader).where(Reader.id == reader_id))
    db_reader = reader_unit.scalars().first()
    if not db_reader:
        raise HTTPException(status_code=HTTPStatus.NOT_FOUND, detail='Пользователь с такими данными не найден.')
    return db_reader

@reader.put('/{reader_id}', response_model=ReaderResponse)
async def update_reader(reader_id: int, data: ReaderUpdate, db: AsyncSession = Depends(get_session), current_user = Depends(get_current_user)):
    """Docstring"""
    reader_unit = await db.execute(select(Reader).where(Reader.id == reader_id))
    db_reader = reader_unit.scalars().first()
    if not db_reader:
...
async def delete_reader(reader_id: int, db: AsyncSession = Depends(get_session), current_user=Depends(get_current_user)):
    """Docstring"""
    reader_unit = await db.execute(select(Reader).where(Reader.id == reader_id))
    db_reader = reader_unit.scalars().first()
    if not db_reader:
